...
/*
	...
	Example:
	(begin example)
	[group this, "id1"] call fpa_ai_fnc_cache;
	(end)
...
params [["_units", [], ["", objNull, grpNull, []]], ["_id", "", [""]]];

private _isReCache = _units isEqualType "";
if (_isReCache) then {
	_id = _units;
	_units = [GVAR(cachedGroups), _id] call CBA_fnc_hashGet;
} else {
...
if (_isReCache) exitWith {
	true
};
private _existingUnits = [GVAR(cachedGroups), _id] call CBA_fnc_hashGet;

if (count _existingUnits == 0) exitWith {
	[GVAR(cachedGroups), _id, _units] call CBA_fnc_hashSet;
	true
};
...
[GVAR(cachedGroups), _id, _existingUnits] call CBA_fnc_hashSet;
