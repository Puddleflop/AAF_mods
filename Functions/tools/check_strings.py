...
def getDefinedStrings(filepath):
    ...
    with open(filepath, 'r', encoding="latin-1") as file:
        content = file.read()
        srch = re.compile('Key ID\=\"(STR_LRMSS_[_a-zA-Z0-9]*)"', re.IGNORECASE)
...
def getStringUsage(filepath):
    ...
    with open(filepath, 'r') as file:
        ...
        srch = re.compile('(STR_LRMSS_[_a-zA-Z0-9]*)', re.IGNORECASE)
        fileStrings = srch.findall(content)

        srch = re.compile('[^EB][CL]STRING\(([_a-zA-Z0-9]*)\)', re.IGNORECASE)
        modStrings = srch.findall(content)
        for localString in modStrings:
            fileStrings.append("STR_LRMSS_{0}_{1}".format(selfmodule, localString))
        ...
        for (subString) in subStrings:
            fileStrings.append(f"STR_LRMSS_{submodule}_{subString}")
